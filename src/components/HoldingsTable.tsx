...
const aggregatePortfolioByCode = (items: Holding[]): Holding[] => {
    ...
    items.forEach(item => {
        ...
        const acquisitionPrice = totalQuantity > 0
            ? ((existing.acquisitionPrice * existing.quantity) + (item.acquisitionPrice * item.quantity)) / totalQuantity
            : 0;
        const sources = new Set(`${existing.source},${item.source}`.split(',').map(source => source.trim()).filter(Boolean));
    ...
    return Array.from(map.values()).sort((a, b) => a.code.localeCompare(b.code, 'ja', { numeric: true }));
};
...
export function HoldingsTable({ isSampleMode = false, onDataUpdate, onUpgradeClick }: HoldingsTableProps) {
    ...
    const handleDeleteHistory = async (entry: PortfolioHistory) => {
        const dateLabel = new Date(entry.createdAt).toLocaleString('ja-JP');
        if (!confirm(`${dateLabel}の履歴を削除しますか？\nこの操作は取り消せません。`)) return;

        setIsHistoryLoading(true);
        try {
            const result = await deletePortfolioHistory(entry.id);
            if (!result.success) {
                alert(result.message);
                return;
            }
        ...
        <div className="bg-white rounded-3xl p-6 shadow-xl border border-indigo-50 overflow-hidden flex flex-col h-[650px]">
            ...
            <div className="mb-3 flex flex-shrink-0 gap-2 border-b border-slate-100">
                <button
                    ...
                    className={`flex items-center gap-2 border-b-2 px-3 py-2 text-sm font-bold transition-colors ${activeTab === 'current'
                ...
                <button
                    ...
                    className={`flex items-center gap-2 border-b-2 px-3 py-2 text-sm font-bold transition-colors disabled:cursor-not-allowed disabled:opacity-40 ${activeTab === 'history'
                        ? 'border-indigo-600 text-indigo-700'
                        : 'border-transparent text-slate-400 hover:text-slate-600'
                        }`}
                >
                    ...
                    {history.length > 0 && (
                        <span className="rounded-full bg-slate-100 px-2 py-0.5 text-[10px] text-slate-600">{history.length}</span>
                    )}
                </button>
            </div>
                ...
                <div className="min-h-0 flex-grow overflow-auto rounded-xl border border-slate-100 bg-slate-50/60 p-4">
                    {isHistoryLoading && history.length === 0 ? (
                        <div className="flex h-full min-h-48 items-center justify-center gap-2 text-sm text-slate-500">
                            <RefreshCcw className="h-5 w-5 animate-spin text-indigo-600" /> 履歴を読み込み中...
                        </div>
                    ) : historyError ? (
                        <div className="rounded-xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-800">
                            {historyError}<br />Supabaseで履歴テーブルのSQLを実行すると利用できます。
                        </div>
                    ) : history.length === 0 ? (
                        <div className="flex h-full min-h-48 flex-col items-center justify-center text-center text-slate-400">
                            ...
                            <p className="font-bold text-slate-500">履歴はまだありません</p>
                            <p className="mt-1 text-xs">次回のSBI・楽天CSVインポートから自動保存されます。</p>
                        </div>
                    ) : (
                        <div className="space-y-3">
                                    ...
                                    <div key={entry.id} className="flex flex-col gap-3 rounded-xl border border-slate-200 bg-white p-4 shadow-sm sm:flex-row sm:items-center sm:justify-between">
                                        <div className="min-w-0">
                                            <div className="flex flex-wrap items-center gap-2">
                                                <span className={`rounded-full px-2.5 py-1 text-xs font-bold ${entry.source === 'SBI'
                                        ...
                                        <div className="flex flex-shrink-0 gap-2">
                                            <button
                                                ...
                                                className="inline-flex items-center gap-1.5 rounded-lg bg-indigo-100 px-3 py-2 text-xs font-bold text-indigo-700 hover:bg-indigo-200"
                                            >
                                                <GitCompare className="h-4 w-4" /> 表示・比較
                                            </button>
                                            <button
                                                type="button"
                                                onClick={() => void handleDeleteHistory(entry)}
                                                className="inline-flex items-center gap-1.5 rounded-lg bg-rose-50 px-3 py-2 text-xs font-bold text-rose-600 hover:bg-rose-100"
                                            >
